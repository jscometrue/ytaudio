...
**모바일에서 예전 화면(URL 입력창 등)이 보일 때**  
배포가 반영됐는데도 모바일에서 옛 버전이 보이면 **캐시** 때문일 수 있습니다.  
→ 브라우저에서 해당 사이트 **캐시 삭제** 또는 **앱(홈 화면 추가한 것) 삭제 후 다시 접속해 홈 화면에 추가**.  
또는 주소창에 **같은 URL을 다시 입력**해 새로고침해 보세요. (서비스 워커 버전을 올려 두었으므로 다음 접속부터는 최신이 나올 수 있습니다.)
